...
#' @export 
#' @importFrom youthvars make_tfd_repln_ds_dict_r3 make_final_repln_ds_dict
...
make_eq5d_ds_dict <- function (data_tb = make_fake_eq5d_ds(), predictors_lup = make_psych_predrs_lup()) 
{
    dictionary_tb <- youthvars::make_tfd_repln_ds_dict_r3() %>% 
        dplyr::filter(var_nm_chr %in% names(data_tb)) %>% youthvars::make_final_repln_ds_dict(additions_tb = ready4use::make_pt_ready4use_dictionary(var_nm_chr = c("uid", 
...
#' @export 
#' @importFrom purrr map flatten_dfr map_int map2_dbl map2_chr map2_int
#' @importFrom stats setNames
#' @importFrom tidyr expand
#' @importFrom dplyr mutate left_join n group_by ungroup arrange filter pull select rename everything rename_with case_when
    ...
    seed_1L_int = 1234, sample_from_1L_int = 10000) 
{
    ...
    k10_lup_tb <- tibble::tibble(K10_int = (9.5 + rexp(sample_from_1L_int, 
        rate = 0.18)) %>% purrr::map_int(~as.integer(min(max(.x, 
        10), 50))), predd_eq5d_dbl = purrr::map2_dbl(K10_int, 
        rnorm(sample_from_1L_int, 0, 0.075), ~predict_utl_from_k10(.x, 
            eq5d_error_1L_dbl = .y)[2]), match_idx_int = purrr::map_int(predd_eq5d_dbl, 
        ~which.min(abs(data_tb$total_eq5d - .x))))
    data_tb <- dplyr::left_join(k10_lup_tb, data_tb %>% dplyr::mutate(match_idx_int = 1:dplyr::n()))
    data_tb <- data_tb %>% dplyr::mutate(Timepoint = total_eq5d %>% 
        purrr::map2_chr(runif(sample_from_1L_int), ~{
            p_1L_dbl <- ifelse(.x > median(data_tb$total_eq5d), 
                0.55, 0.45)
            ifelse(.y > p_1L_dbl, "FUP", "BL")
        })) %>% dplyr::group_by(Timepoint) %>% dplyr::mutate(uid = 1:dplyr::n()) %>% 
        dplyr::ungroup() %>% dplyr::arrange(uid, Timepoint)
    bl_uids_chr <- data_tb %>% dplyr::filter(Timepoint == "BL") %>% 
        dplyr::pull(uid)
    fup_uids_chr <- sample(bl_uids_chr, round((prop_with_fup_data_1L_dbl * 
        length(bl_uids_chr)), 0))
    data_tb <- data_tb %>% dplyr::filter(uid %in% bl_uids_chr) %>% 
        dplyr::filter(Timepoint == "BL" | uid %in% fup_uids_chr)
    data_tb <- data_tb %>% dplyr::mutate(Psych_well_int = faux::rnorm_pre(K10_int, 
        mu = 69.9, sd = 9.9, r = -0.56) %>% round(0) %>% purrr::map_int(~min(.x, 
        90) %>% max(18) %>% as.integer()))
    data_tb <- data_tb %>% dplyr::select(uid, Timepoint, MO, 
        SC, UA, PD, AD, K10_int, Psych_well_int)
    ...
    demog_data_tb <- replication_popl_tb %>% youthvars::transform_raw_ds_for_analysis() %>% 
        ...
        dplyr::select(uid, d_interview_date, d_age, Gender, d_sex_birth_s, 
            ...
            d_studying_working) %>% dplyr::rename(data_collection_dtm = d_interview_date)
    data_tb <- dplyr::left_join(data_tb %>% dplyr::filter(uid %in% 
        demog_data_tb$uid), demog_data_tb) %>% dplyr::select(uid, 
        Timepoint, data_collection_dtm, d_age, Gender, d_sex_birth_s, 
        d_sexual_ori_s, d_relation_s, d_ATSI, CALD, Region, d_studying_working, 
        dplyr::everything()) %>% dplyr::rename_with(~stringr::str_c("eq5dq_", 
        .), .cols = c("MO", "SC", "UA", "PD", "AD")) %>% dplyr::mutate(data_collection_dtm = dplyr::case_when(Timepoint == 
        "FUP" ~ data_collection_dtm + lubridate::days(120), T ~ 
        data_collection_dtm))
    data_tb <- data_tb %>% dplyr::mutate(d_age = dplyr::case_when(Timepoint == 
        "FUP" ~ d_age %>% purrr::map2_int(runif(nrow(data_tb)), 
...
#' @importFrom rlang sym
make_fake_ts_data <- function (outp_smry_ls, depnt_vars_are_NA_1L_lgl = T) 
{
    ...
    knit_pars_ls <- purrr::pmap(list(predr_vars_nms_ls, split(lab_idx_dbl, 
        ...
        ~{
            ...
            mdl_ttls_chr <- paste0(..1[1], ifelse(is.na(..1[2]), 
                ...
                purrr::map_chr(~paste0(ready4fun::get_from_lup_obj(mdl_types_lup, 
                  match_var_nm_1L_chr = "short_name_chr", match_value_xx = .x, 
                  target_var_nm_1L_chr = "mixed_type_chr", evaluate_lgl = F), 
                  " with ", ready4fun::get_from_lup_obj(mdl_types_lup, 
                    match_var_nm_1L_chr = "short_name_chr", match_value_xx = .x, 
    ...
    version_1L_chr = NULL) 
{
    ...
    ttu_lngl_ls = list(best_mdls_tb = tibble::tibble(model_type = mdl_types_chr %>% 
        purrr::map_chr(~ready4fun::get_from_lup_obj(spine_of_results_ls$outp_smry_ls$mdl_types_lup, 
            match_value_xx = .x, match_var_nm_1L_chr = "short_name_chr", 
            target_var_nm_1L_chr = "mixed_acronym_chr", evaluate_lgl = F)), 
        link_and_tfmn_chr = mdl_types_chr %>% purrr::map_chr(~ready4fun::get_from_lup_obj(spine_of_results_ls$outp_smry_ls$mdl_types_lup, 
            match_value_xx = .x, match_var_nm_1L_chr = "short_name_chr", 
            target_var_nm_1L_chr = "with_chr", evaluate_lgl = F)), 
