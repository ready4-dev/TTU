...
descriptives_BL_tb <- tibble::tibble(fkClientID = aqol6d_adol_pop_tbs_ls$bl_outcomes_tb$fkClientID,
  ...
  dplyr::mutate(d_sex_birth_s = dplyr::case_when(as.integer(d_gender) %in% c(1L,2L) & runif(1068)>0.995 ~ as.integer(d_gender) %>% purrr::map_int(~switch(as.integer(.x),2L,1L)),
                                                 #d_gender ==3 ~ 1,
