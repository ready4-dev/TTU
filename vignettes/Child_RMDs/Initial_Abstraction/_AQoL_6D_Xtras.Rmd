...
```{r}
library(ggfortify)
fit = glm( aqol6d_total_w  ~ PHQ9, 
           data = baseline,family=quasibinomial(logit))
summary(fit)
...
ggplot2::ggplot(baseline,ggplot2::aes(x = PHQ9, y= aqol6d_total_w )) +
  ...
  ggplot2::geom_line(data=newdata,ggplot2::aes(x = PHQ9, y= aqol6d_total_w),col="red")  +
  ggplot2::theme_bw() + 
  ggplot2::labs(x="PHQ9 total score",y="AQoL-6D utility score")
...
ggplot2::ggplot(baseline,
                ggplot2::aes(x = PHQ9, y= aqol6d_total_w )) +
  ggplot2::geom_point() + 
  ggplot2::geom_smooth(method = "loess", size = 1.5) +
  ggplot2::geom_line(data=newdata,ggplot2::aes(x = PHQ9, y= aqol6d_total_w),col="red")  +
  ggplot2::theme_bw() + 
...
baseline %>% 
    dplyr::mutate(Simulated=1-exp(-exp(predict(fit)+rnorm(dplyr::n(),0,sigma(fit)))), 
         Original=aqol6d_total_w) %>%  
    tidyr::gather(variable, value, Simulated, Original) %>%
    ggplot(ggplot2::aes(x = value, fill = variable)) +
    ggalt::geom_bkde(alpha=0.5) +
    ggplot2::geom_rug() +
    viridis::scale_fill_viridis( discrete = TRUE) +
    ggplot2::theme_bw() +
...
baseline %>% 
     dplyr::mutate(Predicted=predict(fit,type="response"), 
         Original=aqol6d_total_w) %>% 
  ggplot(ggplot2::aes(x = Original, y=Predicted)) +
  ggplot2::geom_point() + 
  ggplot2::geom_smooth(method = "loess", size = 1.5) +
  ggplot2::theme_bw() +
  ggplot2::geom_abline(intercept = 0, slope = 1, color="red", 
...
ggplot(baseline,ggplot2::aes(x = PHQ9, y= aqol6d_total_w )) +
  ggplot2::geom_point() + 
  ggplot2::geom_smooth(method = "loess", size = 1.5) +
  ggplot2::geom_line(data=newdata,ggplot2::aes(x = PHQ9, y= aqol6d_total_w),col="red")  +
  ggplot2::theme_bw() + ggplot2::labs(x="PHQ9 total score",y="AQoL-6D utility score") 
...
## Random Forest
RF <- randomForest::randomForest(aqol6d_total_w~.,data=baseline,importance=TRUE)
varImpPlot(RF)


## Boruta 
boruta.model <- Boruta::Boruta(aqol6d_total_w~., data = baseline , maxRuns = 300)
...
#                      Rsuqare=caret::R2(baseline$aqol6d_total_w, predict(model), form = "traditional"),
...
summary(fit)
baseline %>% 
    dplyr::mutate(Predicted=simulate(fit)$sim_1, 
         Original=aqol6d_total_w) %>% 
    tidyr::gather(variable, value, Predicted, Original) %>%
    ggplot(ggplot2::aes(x = value, fill = variable)) +
    ggalt::geom_bkde(alpha=0.5) +
