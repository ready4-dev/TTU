...
aqol_items_props_tbs_ls <- make_aqol_items_props_tbs_ls()
```
...
```{r}
tbs_ls <- make_synth_series_tbs_ls(synth_data_spine_ls, series_names_chr = c("bl_part_1_tb","fup_part_1_tb"))

```

```{r, echo=FALSE}
tbs_ls <- add_corrs_and_uts_to_tbs_ls_ls(tbs_ls,
                                         aqol_scores_pars_ls = aqol_scores_pars_ls,
                                         aqol_items_props_tbs_ls = aqol_items_props_tbs_ls,
                                         temporal_corrs_ls = list(aqol6d_total_w = 0.85,
                                                                  aqol_scores = 0.9), 
  ...
  dplyr::mutate(d_gender = d_gender %>% as.character() %>%
                ...
                d_country_bir_s = CALD %>%
                  ...
                  purrr::map_chr(~ifelse(.x,
                                         "No",
                                         "Yes")),
                bads_total = bads_total %>% 
                  purrr::map_dbl(~min(.x,150) %>% max(0)),
                k6_total = k6_total  %>%
                          purrr::map_dbl(~min(.x,24) %>% max(0)),
                phq9_total = phq9_total %>%
                          purrr::map_dbl(~min(.x,27) %>% max(0)),
                gad7_total = gad7_total %>%
                          purrr::map_dbl(~min(.x,21) %>% max(0)),
                oasis_total = oasis_total %>%
                          purrr::map_dbl(~min(.x,20) %>% max(0)),
                scared_total =  scared_total %>%
                          purrr::map_dbl(~min(.x,82) %>% max(0)),
                aqol6d_total_w = aqol6d_total_w %>%
                          purrr::map_dbl(~min(.x,1) %>% max(0.1))
                ) %>%
  dplyr::select(-CALD) %>%
  dplyr::select(-Region)
...
```{r}
composite_tb <- make_domain_items_ls(domains_chr = c("IL", "Rel", "MH", "Coping","Pain", "Sense"),
                     q_nbrs_ls = list(1:4,5:7,8:11,12:14,15:17,18:20),
item_pfx_1L_chr = "aqol6d_q") %>%
  add_domain_unwtd_tots_tb(items_tb = composite_tb,
