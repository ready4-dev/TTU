...
smry_of_one_predr_mdl_tb <- purrr::map_dfr(folds_ls,
  ...
  pred_new_dbl <- predict(mdl,newdata=data_tb[.x,]) %>% calculate_dep_var_tfmn(tfmn_1L_chr = tfmn_1L_chr,
...
mdl_types_lup <- dplyr::bind_rows(FBaqol::mdl_types_lup,
  ...
  dplyr::mutate(fn_chr = c(NA_character_,rep("lm",6),rep("glm",4),rep("betareg::betareg",3)),
                ...
                start_chr = c(rep(NA_character_,7),
                              rep("-0.1,-0.1",4),
                              rep("-0.5,-0.1,3",3)),
                control_chr = c(rep(NA_character_,11),rep("betareg.control",3)),
                predict_type = c(rep(NA_character_,7),
                                 rep("response",7)),
                binml_tfm_lgl = c(rep(F,8),rep(T,3),rep(F,3)))
```
...
purrr::map_dfr(mdl_types_chr,
               ~ {
                 ...
                 tfmn_1L_chr <- stringr::str_sub(.x, start = idx_1L_int)
