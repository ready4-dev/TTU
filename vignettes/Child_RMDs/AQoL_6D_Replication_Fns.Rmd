...
transform_tb_to_mdl_inp <- function(data_tb,
  ...
  tfd_for_gsn_log_mdl_tb <- data_tb %>% 
    dplyr::select(dplyr::all_of(dep_var_nm_1L_chr),#"aqol6d_total_w",
                  dplyr::all_of(id_var_nm_1L_chr),#"fkClientID",
                  dplyr::all_of(round_var_nm_1L_chr),#"round",
    ...
    dplyr::mutate(dplyr::across(dplyr::all_of(predictor_vars_nms_chr), 
                    .fns = list(baseline = ~ dplyr::first(.)/100,
                                change = ~ ifelse(!!rlang::sym(round_var_nm_1L_chr)==round_bl_val_1L_chr,
                                                0,
                                                (.-lag(.))/100))))  %>% #aqol6d_total_w_cloglog
...
fit_gsn_log_lnk <- function(data_tb,
  ...
  brm_model_ls <- fit_ts_model_with_brm(data_tb,
    ...
    # 
    # 
    # brms::brm(formula = as.formula(paste0("aqol6d_total_w ~ ",
...
make_smry_of_brm_mdl <- function(brm_model_ls,
  ...
  RMSE <- psych::describe(apply(predictions,
                                1,
                                fn,
                                y_dbl = data_tb %>% dplyr::pull(!!rlang::sym(dep_var_nm_1L_chr))),#data_tb$aqol6d_total_w
                          quant = c(.25, .75), 
                          skew = F, 
                          ranges = F)
...
make_brm_model_plts <- function(brm_model_ls,
  ...
  set.seed(seed_1L_dbl)
  data_tb$Predicted <- predict(brm_model_ls)[,1] %>% tfmn_fn() #1-exp(-exp(predict(brm_model_ls)[,1]))
  plot_2_plt <- data_tb %>% 
        ...
        ggplot2::labs(x = dep_var_desc_1L_chr, 
                      y = "Density", fill="")
    plot_3_plt <- rlang::exec(ggplot2::ggplot(data_tb), 
                              ggplot2::aes(x = !!rlang::sym(dep_var_nm_1L_chr), 
                                       y = Predicted,
                                       col = !!rlang::sym(round_var_nm_1L_chr)),
                              size = 1,
                              !!!args_ls) + #ggplot(data_tb) +
      # ggplot2::geom_point(ggplot2::aes(x = !!rlang::sym(dep_var_nm_1L_chr), 
      #                                  y = Predicted,
      #                                  col = !!rlang::sym(round_var_nm_1L_chr)),
      #                     size = 1) +
      ggplot2::theme_bw()  + 
...
fit_clg_log_tfmn <- function(data_tb,
  ...
  # RMSE<-psych::describe( apply(predictions,1,calculate_rmse_tfmn,y_dbl=data_tb$aqol6d_total_w),
...
gsn_log_lnk_mdl_ls <- fit_gsn_log_lnk(Baseline_follow,predictor_vars_nms_chr)
model1 <- make_smry_of_brm_mdl(gsn_log_lnk_mdl_ls,
                               data_tb = Baseline_follow,
                               fn = calculate_rmse,
                               predictor_vars_nms_chr = predictor_vars_nms_chr)
mdl_1_plts_ls <- make_brm_model_plts(data_tb)
