...
library(TTU)
```
## Origin and purpose of the TTU replication dataset
The replication dataset distributed with the TTU package is a purely synthetic (entirely fake) dataset that is nonetheless representative (in terms of constituent variables and their uni-variate and [to some extent] joint distributions) of the original study dataset to which we applied the transfer to utility algorithms included in TTU. The purpose of the replication dataset is two-fold:
...
2) to help users trial the functions in the TTU package before potentially applying these functions to their own data.
