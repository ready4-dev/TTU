...
## Analysis parameters
We create an object that specifies the input parameters for our primary analysis using the `make_input_params` function, principally using the objects created in the preceding steps. We can force the use of specified model types or covariates (rather than allowing the analysis algorithm to select these based on automated criteria) by using the `prefd_mdl_types_chr` and `prefd_covars_chr` arguments.
The `make_input_params` performs a number of checks relating to the naming conventions used in some variables of interest and where necessary (for consistency with reporting templates) modifies the names of these variables. We can also specify how we want to adapt the input parameters (e.g. using different combinations of candidate predictors and covariates) for any supplementary analyses that we intend to run using the `scndry_anlys_params_ls ` argument in conjunction with the `make_scndry_anlys_params` function. 
...
# Run analysis
All analytic steps for our primary and secondary analyses are executed with the following function call.
```{r}
...
# Share results
We can now share the results created in the previous step in an online repository. To do this, you will need to have an account with a Dataverse (https://dataverse.org/) provider (e.g., https://dataverse.harvard.edu/). You also need to have created:
 ...
 - an empty dataset (steps 1-3 and 5 in https://guides.dataverse.org/en/5.5/user/dataset-management.html#adding-a-new-dataset)

If you have created an empty dataverse dataset to post results to, assign the DOI for your dataset and the name of the dataverse containing it to an object, `dv_ds_nm_and_url_chr` (these values will be different to those below, which will not work for you). If you do not wish to post results to a dataverse dataset, set `dv_ds_nm_and_url_chr <- NULL`.
...
Next pass these details, along with some paramters created earlier to the `write_study_outp_ds` function. 
```{r echo = TRUE, eval=FALSE }
write_study_outp_ds(dv_ds_nm_and_url_chr = params_ls_ls$path_params_ls$dv_ds_nm_and_url_chr,
...
The dataset populated by the above call can be viewed at: https://doi.org/10.7910/DVN/D74QMP
...
```{r}
study_descs_ls <- make_study_descs_ls(background_1L_chr = "Our study is entirely fictional and has been created to illustrate TTU package functionality.",
...
```{r eval=FALSE}
results_ls <- make_results_ls(dv_ds_nm_and_url_chr = dv_ds_nm_and_url_chr,
                              output_format_ls = output_format_ls,
                              params_ls_ls = params_ls_ls,
                              path_params_ls = path_params_ls,
