...
### Bibliograhic data
We begin by specifying the authorship and other bibliographic data that we wish to attach to all reports that we generate. We use the `ready4_authors_lup` and `ready4_institutes_lup` classes of the [ready4show](https://ready4-dev.github.io/ready4show/) package to do this.
...
### Data dictionary
A data dictionary must also be supplied for the dataset. The dictionary must be of class `ready4_dictionary` from the [ready4use](https://ready4-dev.github.io/ready4use/) package. In this case, we are using a pre-existing dictionary.
...
- If your planned submission format is PDF or LaTeX, edit the markdown files (printing object `results_ls$path_params_ls$paths_ls$path_to_ms_mkdn_1L_dir` will tell you where these have been saved) and then run `write_manuscript(results_ls = results_ls, output_type_1L_chr = "PDF")`. Both PDF and TeX files are saved as a result of this command. Note this call to `write_manuscript` includes a `results_ls = results_ls` argument instead of using the `input_params_ls` argument. That is because, by default a `results_ls` object is saved to a local directory and can thus be retrieved later for use in a new session.

- If your planned submission format is Word, you can just edit the word document rendered by the `write_manuscript` function. This is relatively straightforward, though  unlike the PDF output, you will need to manually edit some of the section numbering and table formatting / headings. If you did not specify "Word" as your intended manuscript output format, you can render a Word version with the following command:
