...
This vignette is an example of implementing of analysis, reporting and sharing workflow to develop transfer to AQoL-6D health utility models. Some of the initial steps in this example are described in more detail in the [EQ-5D example](CSP_EQ5D_TTU.html). This example uses entirely fake data (so do not use its outputs to inform decision making) but was based on a method that was applied to real data (see https://www.medrxiv.org/content/10.1101/2021.07.07.21260129v2.full).
...
```
The document exported to the dataset by the preceding call is accessible at: https://dataverse.harvard.edu/api/access/datafile/4933736 (it is in ".docx" format as we specified "Word" as our desired output format (see earlier in the workflow)). You can override the output format specified in the input parameter list object by using the `output_type_1L_chr` argument.
```{r eval = F}
