...
## Analysis parameters
We create a summary object that contains the core parameters that will be common to most of the analyses we are planning to run, principally using the objects created in the preceding steps. The `make_valid_params_ls_ls` checks the naming conventions used in some variables of interest and where necessary (for consistency with reporting templates) modifies the names of these variables.

```{r eval = F }
params_ls_ls <- make_input_params(ds_tb,
                                  ds_descvs_ls = ds_descvs_ls,
                                  dv_ds_nm_and_url_chr = c("fakes", # Name of dataverse containing dataset
