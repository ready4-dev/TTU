...
```
We now specify the paths to the input data and data dictionary. For this example we are going to use a slightly trimmed down version of the TTU replication dataset and its dictionary[which are described in another vignette article](Replication_DS.html).
...
```
We can now read the data files into local memory.
...
```
The dataset we are using has `r nrow(scored_data_tb)` records on `rlength(scored_data_tb$fkClientID %>% unique())`) study participants. The first six records are reproduced below.
```{r }
scored_data_tb %>%
    head() %>%
...
We next create and object `ds_smry_ls` to store a description of some structural properties of the dataset such as:
 ...
 - the names of the variables variables for unique participant identified and data collection round; and
...
We now create a lookup table with summary information on candidate predictors and add it to the  `ds_smry_ls` object.
...
We next create another lookup table (or as in the below example, use an existing lookup table) with summary information about the types of models we might test.
...
```
We now compare all candidate model types, each using only the highest correlated candidate predictor (`r predr_var_desc_1L_chr`). The `write_sngl_predr_multi_mdls_outps` function will write a summary of each model to a new sub-directory of `r path_to_write_to_1L_chr`, which takes the name passed to the `new_dir_nm_1L_chr` argument. 
...
```
```{r include=F}
# paste0(path_to_write_to_1L_chr,"/","A_Candidate_Mdls_Cmprsns") %>% list.files(pattern = "\\.png$", full.names = T) %>%
# knitr::include_graphics()
```

The function also outputs a table summarising the performance of each of the candidate models.
```{r}
...
We can override the selections based on this criterion and instead incorporate other considerations (possibly based on visual inspection of density plots). We do just this in the following command, specifying new preferred model types, in descending order of preference.

```{r}
mdl_smry_ls$prefd_mdl_types_chr <- c("GLM_GSN_LOG","OLS_CLL")
```

Having identified our preferred model types we can now compare all of candidate predictors using the most preferred model type.
...
mdl_smry_ls$predr_cmprsns_tb %>%
    ready4show::print_table(output_type_1L_chr = "HTML",
                          ...
                          use_lbls_as_col_nms_1L_lgl = T)
```
Now, we prepare a summary table of the performance of each model of the most preferred model type  for each candidate predictor.
...
```
We can override the covariates to select (in the below example we have opted not to do so and continue to use the significant predictors selected by the algorithm in the previous step).
...
We now conclude our model testing by finalising the list of the models that we intend to use when exploring the time series data.
```{r}
...
```
In preparation for the time series modelling, we create a composite object `outp_smry_ls` that contains the key input and output data used in testing models, predictors and covariates.
```{r}
...
```
## Apply selected models to selected predictors in time series models
```{r}
