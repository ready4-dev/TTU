...
```{r child=c('_Reporting_Params.Rmd')}
```
...
```{r}
mdl_smry_ls <- TTU::make_mdl_smry_ls(mdl_types_lup = TTU::get_cndts_for_mxd_mdls(),
                                     folds_1L_int = 10L,
...
path_params_ls <- TTU::make_path_params_ls(use_fake_data_1L_lgl = use_fake_data_1L_lgl,
...
```{r echo = TRUE }
dv_ls <- list(primary_dv_chr = c("fakes", # REPLACE WITH DATAVERSE IN WHICH YOUR DATASET IS LOCATED
...
## Report results
[A report, summarising all the transfer to utility models](https://dataverse.harvard.edu/api/access/datafile/4806542) created in the preceding step is authored with the following function call. 
```{r echo = TRUE, eval=FALSE }
...
If details to a valid online data repository have been supplied, the following function call uploads shareable versions (ie source dataset copies have been replaced with synthetic data) to that dataset. The resulting dataset is available at: https://doi.org/10.7910/DVN/612HDC
```{r echo = TRUE, eval=FALSE }
if(!is.null(dv_ls)){
...
```{r echo = TRUE, eval=FALSE }
TTU::write_to_delete_ds_copies(path_params_ls$paths_ls)
```
