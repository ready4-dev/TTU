...
The steps in this reporting workflow should only be undertaken once an optimal set of input values have been identified by an [initial exploratory analysis](Model_TTU.html). Most of the steps in this example are described with minimal explanation here as they are discussed in more detail in the [AQoL-6D TTU reporting workflow vignette](CSP_AQOL6D_TTU.html), and . Note, this vignette uses fake data - it is for illustrative purposes only and should not be used to inform decision making.
...
### Dataset
Note in this example we modify the variable names of our candidate predictors to ensure they are capitalised, as we wish for these variable names to be capitalised in the final report.

```{r message=FALSE}
ds_tb <- readRDS(url("https://dataverse.harvard.edu/api/access/datafile/4750597")) %>%
  dplyr::rename(K10_int = k10_int,
                Psych_well_int = psych_well_int) 
```
...
The draft manuscript created by the previous call is available at: https://dataverse.harvard.edu/api/access/datafile/4942297 .
