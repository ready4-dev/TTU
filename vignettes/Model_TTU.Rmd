...
```
We now specify the paths to the input data and its data dictionary. For this example we are going to use a slightly trimmed down version of the TTU replication dataset and its dictionary [which are described in another vignette article](Replication_DS.html).
...
The models that are tested are written to the "A_Candidate_Mdls_Cmprsns" sub-directory of "Output" along with five plots for each model. The plots for the first model we tested are reproduced below.

```{r }
paste0(path_to_write_to_1L_chr,"/","A_Candidate_Mdls_Cmprsns") %>% list.files(pattern = "\\.png$", full.names = T) %>% `[`(1:5) %>%
...
### Compare all candidate predictors using preferred model
...
```
The previous call to the `write_mdl_type_multi_outps`{.R} function saves the tested models along with the two plots for each model in the "C_Predrs_Sngl_Mdl_Cmprsns" sub-directory of "Output". The plots for the model using the best performing predictor (`mdl_smry_ls$predr_var_desc_1L_chr`) are
reproduced below. 
...
The previous call to the `write_mdl_type_multi_outps`{.R} function saves the tested models along with the two plots for each model in the "E_Predrs_W_Covars_Sngl_Mdl_Cmprsns" sub-directory of "Output". The plots for the model using the best performing predictor (`mdl_smry_ls$predr_var_desc_1L_chr`) are
reproduced below. 
...
### Preliminary steps
First we summarise what are the combinations of model types, candidate predictors and covariates that we will be using.
...
Next we test each of the `r length(mdl_smry_ls$mdl_nms_ls %>% purrr::flatten_chr())` time series models we specified in the previous step. For this goal, we use the `write_ts_mdls_from_alg_outp`{.R} function that makes a call to the Stan program. Note that for this example we have run only 500 iterations - in practice this number would likely be several times higher. A limitation of the current version of the `write_ts_mdls_from_alg_outp` function is that it only works with the following two model types: 
